...
df <- read.table(file = url('http://www.esapubs.org/archive/ecol/E084/094/MOMv3.3.txt'), header = FALSE, sep = '\t', na.strings = -999)
...
df <- read.delim(file = url, na.strings = -999, header = FALSE)
...
df2 <- read.delim(file = url, na.strings = -999, 
                  nrows = 100, col.names = df_names, header = FALSE)
...
# Go to the URL of the data
fish_url<- url('http://datadryad.org/bitstream/handle/10255/dryad.34127/Brac%202006%20-%20fish%20data.txt?sequence=1')
# What's the field delimiter?
fish <- read.table(file = fish_url, header = TRUE, sep='\t', skip = 1)
# Is there a column header?
