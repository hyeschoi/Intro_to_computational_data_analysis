...
# vector can contain only one number, so using a list is reasonable.
```
...
```{r}
# Fill in.
gsub('us', 'United States', grep('us', countries, value= TRUE, ignore.case = TRUE), ignore.case = TRUE)
```
...
grep("^A|^Z", countries, value = TRUE)
# parenthesis is important! or ^A|^Z 
```
...
```{r}
# Fill in.
grep('^[AEIOU].*[^AEIOU]$', countries, ignore.case = TRUE, value = TRUE)
```