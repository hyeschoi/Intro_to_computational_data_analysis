...
# is as follows:
ggplot(data = tents, aes(x = 0, y = price)) +
  geom_boxplot()
...
ggplot(data=tents, aes(x=brand)) + geom_histogram(aes(fill=brand))
ggplot(data=tents, aes(x=height)) + geom_density(fill='#169ca799')
ggplot(data=tents, aes(x=0, y=height)) + geom_boxplot(color ='red')
ggplot(data=tents, aes(x=capacity, y=height)) + geom_boxplot(color ='#55674d')
...
ggplot(tents, aes(x= seasons)) + geom_bar(aes(fill = seasons))
...
hw + geom_point(aes(size = price, shape= seasons, color=bestuse)) +
  theme(panel.background= element_rect(fill='#f6dfb3'), 
        plot.background = element_rect(fill= '#50467c'))+
  scale_shape_manual(values=c(14,16,18))+
  scale_color_manual(values=c('#f6be10','#810404','#470202'))+
  scale